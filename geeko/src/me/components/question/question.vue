    ...
    <div :class="!noBorder?'question':'question lastChild'" :id="'question'+(index+1)">
        ...
        <div v-if="type == 'radio'" class="question-answer-list">
            <div v-for="(item, index) in answerList" 
                 :key="index"
                 :class="{'answer': true, 'answer-selected': answerSelect==item.value}"
                 >
                ...
                <input type="radio" :name="title" :value='item.value' :id='item.value' v-model="answerSelect"/>
            </div>
        </div>
        ...
        <div v-if="type == 'checkbox'" class="question-answer-list">
            <div v-for="(item, index) in answerList" 
                 :key="index"
                 :class="{'answer': true, 'answer-checkbox':true, 'answer-selected': answerSelectCheckBox.indexOf(item.value) != -1}"
                 >
                <label :for="item.value">
                    <div class="answer-content">{{index + 1}}. {{item.label}}</div>
                    <div class="answer-select">
                ...
                <input type="checkbox" :name="title" :value='item.value' :id='item.value' v-model="answerSelectCheckBox"/>
            </div>
        </div>

        <div v-if="type == 'textarea'" class="question-answer-list">
            <textarea cols="30" rows="10" v-model="answerSelect"></textarea>
        </div>
            ...
            <div class="selectBox">
                <div class="selectInputBox" @click="openSelect()">
                    ...
                    <div v-if="selectOpen == true" class="selectOptionBox">
                        ...
                        <div :class="{'selectOption':true, 'selectOption-select': selectItem == item.label}"
...
<script type="text/ecmascript-6">
    ...
    export default{
        ...
        watch:{
            answerSelectCheckBox(newV, oldV){
                // console.log(newV, oldV)
                this.$emit("change", {title: this.title, value: newV})
            },
            answerSelect(newV, oldV){
                // console.log(newV, oldV)
                this.$emit("change", {title: this.title, value: newV})
            }
        },
        methods: {
            ...
            optionClick(item){
                ...
                this.selectOpen = !this.selectOpen;
                this.$emit("change", {title: this.title, value: item.value})
            },
            closeItem(e){
