<template>
    ...
    <div>
        <page-header>{{$t("notification.news")}}</page-header>
...
<script type="text/ecmascript-6">
    ...
    export default{
        ...
        computed: {
            notifications(){
                return store.getters['me/newsNotifications']
            },
            skip(){
                return store.getters['me/newsNtSkip']
            },
            loaded(){
                return store.getters['me/newsNtLoaded']
            },
            empty(){
                return this.loaded && (!this.notifications || !this.notifications.length)
            },
            finished(){
                return store.getters['me/newsNtFinished']
            },
        },
        methods: {
            listingHandle(){
                this.loading = true
                store.dispatch("me/getNewsNotifications", {skip: this.skip}).then(() => {
                    this.loading = false
                    store.dispatch("me/getNewsNtSkip")
                })
            },
            goPage(){
