...
export default class extends React.Component {
    ...
    render() {
        ...
        return <ALSOLIKES innerRef={this.alsoRef.bind(this)} data-source type="shopping_cart_match_with" data-column="shopping_cart_match_with" data-title="shoppingcart" data-type="shopping_cart_match_with" data-content="You Might Like to Fill it With" data-position="2">
            <div className="__hd">
                ...
                recs && recs.length > 1 && <Swiper {...initialOpts} style={{marginBottom: 12}}>
                    {
                        recs && recs.map((rec, index) => <SwiperSlide style={{width:'auto'}} key={rec.name}>
                           <TAB onClick={evt => {this.setState({selectedRec: rec})}} className={`${rec.name === this.state.selectedRec.name?'active':''}`}>{rec.name}</TAB>
                        </SwiperSlide>)
                    }
                </Swiper>
            }
