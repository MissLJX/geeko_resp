...
const PromotionHead = ({promotion, selected, selectHandle}) => <SelectLine style={{paddingTop: 20, borderTop: '1px dashed #e6e6e6'}}>
...
const BirdHead = ({promotion, selected, selectHandle}) => <SelectLine style={{paddingTop: 20, borderTop: '1px solid #e6e6e6'}}>
	<div className="x-table __hd __vm x-fw">
		<div className="x-cell" style={{width: 30}}>
			<CheckBox onClick={ () => { selectHandle(!selected) } } className={selected ? 'selected' : ''}/>
		</div>
		<div className="x-cell">
			<div><span dangerouslySetInnerHTML={{__html: promotion.title}}/><Ask style={{marginLeft: 4}} message={promotion.description} /></div>
		</div>
	</div>
</SelectLine>
...
const Promotion = class extends React.Component {
	...
	render () {
		...
		return <div>
			{ delivery.combinatorialPromotion && <PHead selectHandle={ this.groupClick.bind(this) } selected={this.selected()} promotion={delivery.combinatorialPromotion}/> }
