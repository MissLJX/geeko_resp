    ...
    <div class="mainArea">
        <div class="model-fixed">
            <div class="swiper-top">
                <div :class="{ active : scrollActive }" v-if="modelActive" data-name="model" @click="sizeScrollTop('size')">{{$t('label.model_stature')}}</div>
                <div :class="{ active : !scrollActive }" :data-name="modelActive ? 'size' : ''" v-if="sizeshow" @click="sizeScrollTop('model')">{{$t('label.size_chart')}}</div>
            </div>
        </div>
        ...
        <div class="total">
            <div class="model stature" v-show="modelActive" ref="modelStature">
            ...
            <div class="model" v-if="sizeshow" ref="model">
    ...
    export default {
        ...
        methods:{
            handleScroll:function(e){
                ...
                let offsetTop = this.$refs.model.offsetTop;
            ...
            sizeScrollTop:function(name){
                let top = document.documentElement.scrollTop || document.body.scrollTop;
                let referenceTop = (this.$refs.model.offsetTop - 48);

                if(name === 'model'){
                    const timeTop = setInterval(() => {
                        let intervalTop = top += 20;
                        if(intervalTop >= referenceTop){
                            document.documentElement.scrollTop = referenceTop;
                            clearInterval(timeTop)
                        }else{
                            document.documentElement.scrollTop = intervalTop;
                        }
                    }, 15)
                }else{
                    const timeTop = setInterval(() => {
